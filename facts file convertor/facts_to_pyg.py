...
def convert_contract(fact_dir, label):
    """將合約的 .facts 文件轉換為 PyTorch Geometric Data 對象"""
    # 讀取基本塊結構
    ...
    for blk in block_heads:
        ...
        # 統計特徵
        feat.append(len(ops))  # 指令數量
        feat.append(entropy_from_counts(counter))  # 操作碼熵
        feat.append(danger_count / max(1, len(ops)))  # 危險操作比例
        feat.append(1.0 if blk in entries else 0.0)  # 是否為入口塊
        feat.append(1.0 if blk in exits else 0.0)  # 是否為出口塊
        feat.append(block_id[blk] / num_nodes)  # 標準化的塊位置
